...
$(function(){
	...
	// jQuert('태그선택자').실행할메소드(....); 	=> jQuery 는 줄여서 $ 로 표기함
...
function changeLogin(element) {
	...
	var userid = element.name.substring(8);	//loginok_ 제외한 뒷글자 모두가 회원 아이디
	console.log('userid : ' + userid);
	
	if(element.checked == true && element.value == 'false'){	// 로그인 제한이 체크되었다면
		console.log('로그인 제한이 체크됨');
		location.href = "${ pageContext.servletContext.contextPath }/loginok.do?userId=" + userid + "&loginOk=N";
	}else if(element.checked == true && element.value == 'true'){		// 로그인 가능이 체크되었다면
...
<form action="msearch.do" id="ulogokform" class="sform" method="get" >
	...
	<fieldset>
		<legend>검색할 로그인제한 또는 가능을 선택하세요.</legend>
			<input type="radio" name="keyword" value="Y"> 로그인 가능 회원만 조회 &nbsp;
			<input type="radio" name="keyword" value="N"> 로그인 제한 회원만 조회 &nbsp;
			<input type="submit" value="검색">
	</fieldset>
</form>
