...
function dupIdCheck(){
	...
	//서버측에 요청하고 응답받을 때 페이지 새로고침 되면 안됨 - 해결 : ajax 기술
	...
	$.ajax({
		...
		<%-- 응답오는 값이 문자열이면, dataType 옵션 생략 가능 --%>
...
function validate(){
	...
	if(pwdValue !== pwdValue2){
		...
		document.getElementById('userPwd').value = ''; 	 //기록된 값 지우기 javascript
		$('#userPwd2').val('');		//기록된 값 지우기	jQuery
...
//제이쿼리 : $(function(){ 코드 }); == 축약형 jQuery(document).ready(function(){});
window.onload = function(){
	...
	photofile.addEventListener('change', function(event){
		...
		//현재 선택한 사진파일은 클라이언트 컴퓨터에 있는 파일임, src 속성에 적용 못 함
		
		//클라이언트 컴퓨터에 있는 사진파일을 읽어들여서 출력되게 처리해야 함
		//파일입력 -- 출력처리
		const reader = new FileReader(); 	//자바스크립트의 파일읽기 객체 생성
		//이벤트 콜백함수 실행구문(기존 방식) : reader.onload = function(event){ 읽어들이기 처리 };
		//람다(lambda) 스트림 방식 사용
...
<br>
<%-- form 에서 파일이 첨부되어서 전송이 될 경우에는, 반드시 enctype="multipart/form-date" 속성을 추가해야 함 --%>
...
<table id="outer" align="center" width="700px" cellspacing="5" cellpadding="0">
	...
	<tr><th>사진첨부</th>
		<td>
		<%-- 사진을 파일로 첨부해서 전송한다면, input type="file" 로 지정하면 됨 : 사진 미리보기 불가 --%>
		<%-- <input type="file" name="photoFileName"> --%>
		<%-- 첨부된 사진 미리보기가 되도록 하려면, 사진 미리보기용 영역 지정해야함 : 서버로는 전송 안 됨 --%>
		...
		<%-- 첨부파일의 name 은 필드명(property)과 별개로 지정함 
