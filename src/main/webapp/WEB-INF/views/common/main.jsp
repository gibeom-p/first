...
<%-- page 지시자 태그 안에 errorPage 속성 추가할 수 있음
       errorPage 속성은 이 jsp 페이지에서 에러가 발생하면, 지정한 jsp 페이지로 에러를 넘기면서 페이지를 바꿈
 --%>
<%-- jsp 페이지에서 사용하는 jsp element 중 지시자 태그 (directive tag)에서 page 지시자는 jsp 페이지에 한번만
       사용할 수 있음 page 지시자 태그에서 주로 이 페이지의 설정 정보를 속성으로 처리함
       import 속성만 분리할 수 있음
--%>
...
$(function(){
   //최근 등록된 공지글 3개 (tob-N) 전송받아서 출력 처리
   $.ajax({
      url: 'ntop3.do',   /* 요청 url */
      type: 'post',      /* 전송 방식 */
      dateType: 'json',   /* 컨트롤러로부터 응답받을 데이터의 자료형, 생략되면 string */
...
--%>
	...
	<c:if test="${ !empty sessionScope.loginUser }">
		<div id="loginBox" class="lineA">
			...
			<%-- EL 사용으로 바꾼다면, ${ 컨트롤러에서 setAttribute 할 때 저장한 이름.멤버변수명 } --%>
			${ loginUser.userName } 님 &nbsp; <a href="logout.do">로그아웃</a> <br>
			<!-- a 태그로 서블릿 컨트롤러를 요청하면, 전송방식은 get 임 -->
			메일 0, 쪽지 0, 알림 0 <br>
			<%-- a 태그로 서비스 요청 url 작성시, 값도 같이 전송하려면 쿼리 스트링을 사용해야 함 
	...
	<%-- jsp 파일 안에 별도로 작성된 다른 jsp 파일을 포함할 수 있음 
      주의 : 상대경로만 사용할 수 있음
      목적 : 모든 뷰 페이지에 반복 사용(출력)되는 내용이 있다면, 별도의 jsp로 작성해서
            각 페이지에 포함시키면 됨 (중복 배제)
--%>
