	...
	<table align="center" width="650" border="1" cellspacing="0"
		...
		<c:forEach items="${ requestScope.list }" var="board">
			<tr align="center">
				...
				<td align="left">
					<%-- 원글과 댓글, 대댓글을 구분하기 위해 제목글자 앞에 ▶ 표시함 --%> <c:if
						test="${ board.boardLev eq 2 }">
            &nbsp; ▶
            </c:if> <c:if test="${ board.boardLev eq 3 }">
            &nbsp; &nbsp; ▶▶
            </c:if> <c:url var="bd" value="bdetail.do">
						<c:param name="bnum" value="${ board.boardNum }" />
						<c:param name="page" value="${ nowpage }" />
					</c:url> <a href="${ bd }">${ board.boardTitle }</a>
				</td>
				<td>${ board.boardWriter }</td>
				<td><c:if test="${ !empty board.boardOriginalFileName }">◎</c:if>
					<c:if test="${ empty board.boardOriginalFileName }">&nbsp;</c:if></td>
