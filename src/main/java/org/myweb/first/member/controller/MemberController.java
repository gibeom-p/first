...
@Controller // servlet-context.xml 에 Controller 로 자동 등록되는 어노테이션임
public class MemberController {
	...
	// 로그인 페이지 내보내기용
	@RequestMapping("loginPage.do") // 뷰페이지에서 사용하는 요청 이름 등록용 어노테이션, Mapping Handler 가 관리함
	public String moveLoginPage() {
		return "member/loginPage"; // 뷰리졸버로 리턴됨 >> 뷰리졸버가 지정된 폴더에서 해당 jsp 파일 찾아서 클라이언트로 전송
	}
...
//						// 새 세션객체가 생성되면 자동으로 세션 id 가 세션 객체에 등록됨
//						
//						System.out.println("생성된 세션 객체의 id : " + session.getId());
//						// 자동으로 request 객체에 발급된 세션 id 가 기록됨, 서버와 클라이언트를 왔다갔다함
//						
//						// 자동 로그아웃 타임을 지정할 수도 있음 => 요청이 없을 때 세션 객체 무효화
//						//session.setMaxInactiveInterval(30 * 60);	// 초 단위로 30분 설정함
//						
//						
//						// 세션 객체 안에 로그인한 회원 정보를 기록 저장시킬수도 있음 (뷰페이지에서 꺼내서 사용할 경우)
//						session.setAttribute("loginUser", loginUser); 
//						
//						// 내보낼 뷰 페이지 지정 : 뷰 페이지만 내보내는 경우
...
//		}	// login.do
	...
	// 서버로 전송온 parameter 값들을 저장하는 객체를 command 객체라고 함 (dto 객체)
...
//	}
	...
	public String loginMethod(Member member, HttpSession session, SessionStatus status, Model model) {
		...
		// 로그인 요청은 회원의 아이디로 먼저 회원 정보를 조회해 옴 >> 조회해 온 정보에서 암호를 꺼내서 비교 확인
		Member loginUser = memberService.selectMember(member.getUserId());

		// 조죄해 온 회원 정보가 있고, 암호화된 패스워드와 로그인시 전송온 패스워드가 일치한지 비교
...
//		String userId = request.getParameter("userId");
	 ...
	 * 보내는 방식 or 별도의 출력스트림을 만들어서 내보내기 //방식2, 별도의 출력스트림 연결해서 문자열 내보내기 방식 사용함 (반환자료형
	 * void)
	 * 
	 * int result = memberService.selectCheckId(userId); String resultStr = null;
	 * if(result == 0) { resultStr = "ok"; }else { resultStr = "dup"; }
	 * 
	 * //response 를 이용해서 클라이언트와 출력스트림을 열어서 문자열값 내보내기 할 것임
	...
	public String dupCheckUserIdMethod(@RequestParam("userId") String userId) {
		// 방식1, ResponseBody 에 담아서 문자열 내보내기 방식 사용함 (반환자료형이 String)
...
//	}
	...
	// 업로드되는 파일은 따로 전송받음 => multipart 방식으로 전송옴 => 스프링이 제공하는 MultipartFile 클래스 사용해서
	// 받음
	// 비밀번호(패스워드) 암호화 처리 기능 추가
	@RequestMapping(value = "enroll.do", method = RequestMethod.POST)
	public String memberInsertMethod(Member member, Model model, HttpServletRequest request,
		...
		// -------------------------------------------------------
		// request.getSession().getServletContext() --> context root 까지의 경로
		String savePath = request.getSession().getServletContext().getRealPath("resources/photoFiles");
		// 서버 엔진이 구동하는 웹 어플리케이션(Context)의 루트(webapp) 아래의 "resources/photoFiles" 까지의 경로
		// 정보를 저장
		...
		if (!mfile.isEmpty()) {
			...
			if (fileName != null && fileName.length() > 0) {
				try {
					...
					// 첨부파일 저장시 에러 발생
					e.printStackTrace(); // 개발자가 볼 에러 정보
	...
	// '내 정보 보기' 요청 처리용 메소드
	/*
	 * 컨트롤러 메소드에서 뷰리졸버로 리턴하는 타입은 String (뷰파일명 리턴), ModelAndView 를 사용할 수 있음 클라이언트가 보낸
	 * 데이터 추출은 String 변수 = request.getParameter("전송이름"); 스프링에서는 전송값 추출을 위한 위의 구문과
	 * 동일한 동작을 수행하는 어노테이션을 제공함
	 * 
	 * @RequestParam("전송이름") 자료형 변수명 == request.getParameter("전송이름"); 이 어노테이션은 메소드
	 * () 안에 사용함
	 */
	@RequestMapping("myinfo.do") // 전송방식이 get 일 경우
	public String memberDetailMethod(@RequestParam("userId") String userId, Model model) {
		...
		if (member != null) { // 조회 성공시
			...
			if (member.getPhotoFileName() != null) {
				...
				logger.info("사진 파일명 확인 : " + member.getPhotoFileName() + ", " + originalFileName);
			}
	...
	public String memberUpdateMethod(Member member, Model model, HttpServletRequest request,
		...
		} else { // 새로운 암호가 전송오지 않았다면, 현재 member.userPwd = null 임 => 쿼리문에 적용되면 기존 암호가 지워짐
	...
	public ModelAndView memberListMethod(ModelAndView mv, 
		...
		if(list != null && list.size() > 0) {	// 조회 성공시
			...
			mv.addObject("list", list);		//request.setArrtibute("list", list) 와 같음
	...
	public ModelAndView memberSearchMethod(HttpServletRequest request, ModelAndView mv) {
		...
		if(action.equals("uenroll")) {
			if(begin != null && end != null) {	
				search.setBegin(Date.valueOf(begin));		// 날짜데이터로 바꿔서 search.set 으로 값을 기록해 넣음
				search.setEnd(Date.valueOf(end));
			}
		}else if(keyword != null) {
			if(action.equals("uage")) {
				search.setAge(Integer.parseInt(keyword));	// 정수데이터로 바꿔서 search.age 에 기록해 넣음
			}else {
		...
		//총 페이지수 계산을 위해 검색 결과가 적용된 총 목록 갯수 조회함
		int listCount = 0;
		switch(action) {
		...
		if(list != null && list.size() > 0) {
			mv.addObject("list", list);			//리스트 출력
			mv.addObject("paging", paging);		//페이징처리
			mv.addObject("action", action); 		//검색 종류
